...
def show(graph_states,proc_order,idx):
    graph_state = graph_states[idx]
    G = nx.Graph()
    vertices = set([el[0] for el in proc_order]+[el[1] for el in proc_order])
    if graph_state[0]==0:
        for i in vertices:
            G.add_node(i)
...
def edge_in_initial_ghz(state, v1,v2):
    # print("check edge",state[1],v1,v2)
    v1n = []
    v2n = []
    for el in state[1]:
        if el[0] == v1 and el[1] == v2:
            continue
        if el[0] == v1:
            v1n.append(el[1])
        elif el[1] == v1:
            ...
            v2n.append(el[0])
    # print("v1n",v1n,"v2n",v2n)
    if (len(v1n)+len(v2n)) > 1:
        #v1 and v2 have together more than one neighbor
        return False
    if (len(v1n)+len(v2n)) == 0:
        return True
    
    neighbor = v1n[0] if len(v1n) > 0 else v2n[0]
    for el in state[1]:
        if el[0] == neighbor:
            if not el[1] in [v1,v2]:
                #single neighbor of v1 resp. v2 has another neighbor
                # print("found another neighbor",el[1])
                return False
        elif el[1] == neighbor:
            if not el[0] in [v1,v2]:
                # print("found another neighbor",el[0])
                return False
    return True


def skip_trivial_edges(state, proc_order):
    while True:
        next_edge = find_next_edge(state,proc_order)
        if next_edge==-1: break
        v1,v2 = sorted(proc_order[next_edge])
        #check if edge belongs to graph <=3 nodes and skip those edges
        if edge_in_initial_ghz(state, v1,v2):
...
def get_first_passage_times(proc_order, fusion_sucess_probabilities, verbose=False):
    '''
    ...
    '''
    graph_state = skip_trivial_edges([0,[]], proc_order)
    ...
    def process_state(state, proc_order, numbering):
        # print("state before trivial edge skip",state)
        state = skip_trivial_edges(state, proc_order)
        # print("state after trivial edge skip",state)
        if (state[0],*state[1]) not in numbering:
            dfs([state[0],state[1][:]])
        
        return numbering[(state[0],*state[1])]

    def dfs(state):
        nonlocal cur_idx
        state_orig = [state[0],state[1][:]]
        numbering[(state_orig[0],*state_orig[1])] = cur_idx
        idx_to_state[cur_idx] = [state_orig[0],state_orig[1][:]]
        cur_idx+=1
        idx_orig = numbering[(state_orig[0],*state_orig[1])]

        next_edge = find_next_edge(state,proc_order)
        if next_edge==-1: return
        v1,v2 = sorted(proc_order[next_edge])

        success_state = [state[0],state[1][:]]
        failure_state = [state[0],state[1][:]]
        
        if state[0]==0:            
            if vertex_without_edge(state,v1) or vertex_without_edge(state,v2):

                #success:
                success_state[1].append(tuple(sorted([v1,v2])))
                success_state[1] = sorted(success_state[1])
                idx_transition = process_state(success_state, proc_order, numbering)
                transitions.append([idx_orig,idx_transition,"p"])

                #failure
                failure_state = delete_edge(failure_state, v1,v2)
                idx_transition = process_state(failure_state, proc_order, numbering)
                ...
                #failure
                failure_state = blow_up(failure_state, v1)
                idx_transition = process_state(failure_state, proc_order, numbering)
                transitions.append([idx_orig,idx_transition,"q"])
            ...
            #failure
            failure_state = blow_up(failure_state, v2)
            idx_transition = process_state(failure_state, proc_order, numbering)
            transitions.append([idx_orig,idx_transition,"q"])
    ...
    if verbose:
        ...
        for idx_show in range(len(idx_to_state)):
            show(idx_to_state,proc_order,idx_show)
    """
...
if __name__ == "__main__":
    ...
    fusion_sucess_probabilities = np.linspace(0.5,1.0,10) #or geomspace
    fp_times = get_first_passage_times(proc_order, fusion_sucess_probabilities)
    for k,v in fp_times.items():
        print(k,v)